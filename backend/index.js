...
// NEW: DELETE a column and all its associated tasks
app.delete("/columns/:columnId", (req, res) => {
    ...
    try {
        ...
        const taskIdsInColumn = columns[columnId].taskIds;

        // Delete all tasks in the column
        taskIdsInColumn.forEach(taskId => {
            delete tasks[taskId];
        });
        ...
        res.status(500).json({ message: "Failed to delete column", error: err.message });
    }
});
